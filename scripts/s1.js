...
function setup() {
  ...
  var j = pixelDensity;
  glitchy = 4 * (cam.width * j) * (cam.height / 2 * j);
...
function draw() {
  var draw_position_x1 = windowWidth / 2;
  if (cam) {
    cam.loadPixels();
    for (var y = 0; y < windowHeight; y++) {
      var r = cam2.pixels[4 * (y * cam.width + cam.width / 2)];
      ...
      set(draw_position_x, y, [q, w, r, g, b, a, d, c]);
    }
  }
  updatePixels();
  draw_position_x++;
  ...
  if (cam1) {
    ...
    for (var y1 = 0; y1 < windowHeight; y1++) {
      ...
      set(draw_position_y, y1, [r1, g1, b1, a1, c1]);
    }
    updatePixels();
    // loop back around
    draw_position_x1++;
    if (draw_position_x1 >= windowWidth) {
      draw_position_x1 = windowWidth / 2;
    }
    ...
    if (cam2) {
      ...
      for (var draw_position_y = windowHeight; draw_position_y < 0; draw_position_y++) {
        ...
        set(x2, draw_position_y, [r2, g2, b2, a2, c2]);
      }
      updatePixels();
      // loop back around
      draw_position_y--;
      if (draw_position_y >= windowHeight) {
        draw_position_y = windowHeight / 2;
      }
    }
  }
}